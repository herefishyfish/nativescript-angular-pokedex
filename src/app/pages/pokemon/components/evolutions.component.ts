...
@Component({
  ...
  template: `
    <ng-container *rxFor="let pokemon of pokemon?.species?.evolutions; let last = last">
      <GridLayout rows="auto auto auto">
        <StackLayout orientation="horizontal">
          <Label hlmH4>{{ pokemon?.name | titlecase }}</Label>
          <Label hlmP style="line-height: 0;" class="m-2 mt-0 border-border border-b text-muted-foreground">#{{
            pokemon?.id
          }}</Label>
        </StackLayout>
        <ImageCacheIt row="1" width="100" height="100" style="margin: -8;" [src]="pokemon?.sprite" stretch="aspectFill"></ImageCacheIt>
        <Label *ngIf="!last" text="&#xf078;" row="2" class="fa w-full h-6 bg-accent text-accent-foreground p-0 text-center"></Label>
      </GridLayout>
    </ng-container>
  `,
  standalone: true,
  imports: [NativeScriptCommonModule, RxFor, NgIf, HlmH4Directive, HlmPDirective, ImageCacheItModule],
...
export class PokemonEvolutionsComponent extends StackLayout {
  ...
  ngAfterViewInit() {
    console.log(this.pokemon?.species?.evolutions);
  }
}
